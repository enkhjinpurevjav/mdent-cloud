...
/**
 * Map DiscountPercent enum to a numeric value.
 */
...
/**
 * Map numeric discount to DiscountPercent enum.
 */
...
/**
 * Compute patient balance using invoices + payments.
 */
async function getPatientBalance(patientId) {
  ...
  const paidByInvoice = new Map(payments.map((p) => [p.invoiceId, Number(p._sum.amount || 0)]));
 ...
 * Fetch or lazily create an invoice skeleton for an encounter.
 */
router.get("/encounters/:id/invoice", async (req, res) => {
  ...
  try {
    ...
    console.error(`GET /encounters/${encounterId}/invoice failed:`, err.message);
 ...
 * Create or update an invoice structure for an encounter.
 */
router.post("/encounters/:id/invoice", async (req, res) => {
  ...
  try {
    ...
    const totalBeforeDiscount = normalizedItems.reduce((sum, it) => sum + it.lineTotal, 0);
    const discountRate = discountPercentToNumber(discountEnum) / 100;
    const finalAmount = totalBeforeDiscount * (1 - discountRate);

    const existingInvoice = encounter.invoice;
    if (existingInvoice?.eBarimtReceipt) {
      return res.status(409).json({ error: "Invoice already has an e-Barimt receipt." });
    }
    ...
    console.error(`POST /encounters/${encounterId}/invoice failed:`, err.message);
