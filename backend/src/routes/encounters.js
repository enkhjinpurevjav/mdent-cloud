 ...
 * Sync problem texts for a diagnosis - upsert/delete to match provided list
 */
router.put("/encounter-diagnoses/:id/problem-texts/sync", authenticateJWT, async (req, res) => {
  try {
    const encounterDiagnosisId = Number(req.params.id);
    if (!encounterDiagnosisId || Number.isNaN(encounterDiagnosisId)) {
      return res.status(400).json({ error: "Invalid encounter diagnosis id" });
    ...
    // Get existing problem texts
    const existingTexts = await prisma.encounterDiagnosisProblemText.findMany({
 ...
 * Sync service texts - upsert/delete to match provided list
 */
router.put("/encounter-services/:id/texts/sync", authenticateJWT, async (req, res) => {
  try {
    const encounterServiceId = Number(req.params.id);
    if (!encounterServiceId || Number.isNaN(encounterServiceId)) {
      return res.status(400).json({ error: "Invalid encounter service id" });
