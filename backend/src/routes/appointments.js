 ...
 * Used by GET /api/appointments (calendar & lists).
 */
...
/**
 * For availability we want explicit UTC+8 day boundaries (independent of server TZ)
 */
...
router.post("/", async (req, res) => {
  try {
    ...
    if (Number.isNaN(parsedPatientId) || Number.isNaN(parsedBranchId)) {
      return res.status(400).json({ error: "patientId and branchId must be numbers" });
    }
    ...
    if (endAt !== undefined && endAt !== null && endAt !== "") {
      ...
      if (tmp <= scheduledDate) {
        return res.status(400).json({ error: "endAt must be later than scheduledAt" });
      }
      endDate = tmp;
    }
...
router.post("/:id/start-encounter", async (req, res) => {
  try {
    ...
    if (appt.status !== "ongoing") {
      return res.status(400).json({
        error: 'Зөвхөн "Явагдаж байна" (ongoing) төлөвтэй цаг дээр үзлэг эхлүүлэх боломжтой.',
      });
    }

    if (!appt.patient) return res.status(400).json({ error: "Appointment has no patient linked" });
    if (!appt.doctorId) {
      return res.status(400).json({ error: "Энэ цаг дээр эмч сонгоогүй тул үзлэг эхлүүлэх боломжгүй." });
    }
    ...
    return res.status(500).json({ error: "Failed to start or open encounter for appointment" });
  }
});
...
router.get("/:id/encounter", async (req, res) => {
  try {
    ...
    if (!encounter) {
      return res.status(404).json({ error: "Үзлэг олдсонгүй. Эмч үзлэг эхлүүлээгүй байна." });
    }

    return res.json({ encounterId: encounter.id });
  } catch (err) {
    console.error("GET /api/appointments/:id/encounter error:", err);
    return res.status(500).json({ error: "Үзлэгийн мэдээлэл авах үед алдаа гарлаа." });
  }
});
 ...
 *
 * FIXED: YMD-string loop, stable clinic weekday labels, no Date drift.
 */
